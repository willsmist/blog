...
奇怪的是即使 init <level> 没有成功执行而进入指定的级别，命令 runlevel 输出的却是成功执行了 init <level> 之后的结果，例如
...
不建议使用 init 0 和 init 6 执行关机和重启命令,可能造成某些服务不能正确关闭，不在 root 下执行会不成功。
...
一个 IP 地址可以有65536个端口，0-65535

一般的，0-10000是系统预留端口，10001以上的端口用户可自由使用

### /etc/services

文件 /etc/services 以 service-name port/protocol [aliases] [# comment] 的形式描述了每个服务
...
-a (all)显示所有选项，默认(就是没有-a)不显示LISTEN相关
-t (tcp)仅显示tcp相关选项
-u (udp)仅显示udp相关选项
...
这对应远程连接的两个 XShell 会话, ESTABLISHED 表示正在连接着 CentOS 22 端口号，也就是ssh服务。
...
在 /etc/rc.d/init.d/ 下创建服务管理脚本的软连接
...
通过修改 /etc/rc.d/rc.local 文件添加启动命令来实现

例如使从源码安装的 apache 服务随系统启动而启动
